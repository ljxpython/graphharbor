...
The server SHALL support official `client.runs.stream(..., version="v2")` calls with stream modes, subgraph output, heartbeat, terminal events and resumable replay.

#### Scenario: v2 reconnect
- **WHEN** a client reconnects with its last accepted cursor
