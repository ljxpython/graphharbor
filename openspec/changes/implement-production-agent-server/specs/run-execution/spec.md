...
The system SHALL use a PostgreSQL lease and heartbeat to ensure one active worker claim per run, reclaim expired claims, and retry infrastructure failures no more than three times.

#### Scenario: Worker failure
- **WHEN** a worker dies after claiming a run
- **THEN** the reaper reclaims the lease and the run is retried or terminally errored according to the retry limit
