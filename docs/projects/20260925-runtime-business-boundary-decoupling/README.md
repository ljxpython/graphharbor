...
2026-09-25 实施状态：未执行生产数据删除、迁移或切换。删除 tenant/project SQL 隔离之前，必须先完成全部生产入口授权与跨用户拒绝验证；不能为满足一次性交付诉求先移除安全约束。
...
| 实际 Store 消费者、平台迁移脚本、runtime-service 依赖锁定 | 平台管理 namespace；离线迁移历史归属/幂等/身份快照；切换到新 GraphHarbor，不保留旧包兼容 | D01—D05 |
...
| P3 数据切换 | metadata 回填、幂等迁移、Store 契约切换；可演练回退 | D02—D05 |
...
4. 历史数据默认保留，在停止写入后离线转换并删除旧列；新运行代码不包含双写、双读或旧格式分支。Store 物理 namespace 可由平台继续使用，核心不解析。
5. 幂等键通用化与 Store 对外 namespace 属显式契约切换，不得冒充“无差异内部重构”。

评审记录：2026-09-25，用户明确“我同意，按照规划开始实施”，批准 01—04 按彻底解耦方案实施。保留历史数据，两个仓库同步适配；不自动提交、发布或切换生产环境。
...
已完成源码与版本声明核对、官方文档及本地参照实现阅读；未调用真实模型、业务数据库、生产 API，未执行迁移、发布或功能回归。文档校验结果见专题 04。本轮仅生成计划及导航，不改变服务能力状态。
