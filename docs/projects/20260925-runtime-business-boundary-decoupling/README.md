...
- **状态：** `partial`。Worker 业务字段、workspace 源码及核心 SQL tenant/project scope 已完成代码移除；Thread 创建未知结果可用 UUID 对账。完整路由授权、迁移/回退演练、候选包联合验收和平台关键业务链路仍未完成。
...
2026-09-25 实施状态：未执行生产数据删除、迁移或切换。候选代码已移除 tenant/project SQL 隔离，必须在生产入口授权矩阵、隔离 PG17 迁移/冲突演练和跨用户拒绝路径全部通过后才能发布或切换。
